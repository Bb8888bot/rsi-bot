...
def fetch_futures_klines(symbol, interval, limit=120):
    endpoints = [
    ...
    for url in endpoints:
        try:
            r = requests.get(url, params={"symbol": symbol, "interval": interval, "limit": limit}, headers=headers, timeout=2)
...
def fetch_data_all():
    ...
    p1h = fetch_futures_klines("BTCUSDT", "1h", 120)
    if not p1m:
        raise Exception("Binance API 抓取失败")
    p = p1m[-1]
    r1 = calc_rsi_wilder(p1m, 6)
    r3 = calc_rsi_wilder(p3m, 6) if p3m else r1
    r5 = calc_rsi_wilder(p5m, 6) if p5m else r1
    r10 = calc_rsi_wilder(resample_klines(p1m, 10), 6)
    r1h = calc_rsi_wilder(p1h, 6) if p1h else r1
...
def analyze_high_winrate(p, r1, r3, r5, r10, r1h, boll_up, boll_mb, boll_dn, ema7, ema25, ema99):
    ...
    if p <= boll_dn and r1 <= 15 and r3 <= 25 and r5 <= 30 and not is_strong_down:
        return "🔥【S级绝杀·买入看涨(UP)】", "BOLL下轨极限支撑 + 三重超卖！胜率 90%+", "#00ff41", "S-TIER WIN 90%"
    if p >= boll_up and r1 >= 85 and r3 >= 75 and r5 >= 70 and not is_strong_up:
        return "🔥【S级绝杀·买入看跌(DOWN)】", "BOLL上轨极限阻力 + 三重超买！胜率 90%+", "#ff003c", "S-TIER WIN 90%"
    ...
    return "暴爷无极限事件合约矩阵待命", "盘面无极值共振，保持观望等待高胜率信号", "#848e9c", "STANDBY"
